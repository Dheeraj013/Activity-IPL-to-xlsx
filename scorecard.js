    ...
    for (let i = 0; i < bothInningArr.length; i++) {
        ...
        for (let j = 0; j < batsManTableBodyAllRows.length; j++) {
            ...
            if (numberofTds.length == 8) {
                ...
                console.log(playerName, "played for", teamName, "scored", runs, "in", balls, "with ", fours, "fours and ", sixes, "sixes", "with strike rate", sr);
...
function processPlayer(playerName, teamName, runs, balls, fours, sixes, sr) {
    let obj = {
        playerName,
        teamName,
        runs,
